...
## Phase 1 — Focus Area & Document Selection

### Step 1a — Determine focus area

Before showing document options, determine what type of work the user is
doing so you only show relevant documents. This reduces overwhelm.

**Auto-detect from workspace (preferred):** If USER_ROLE or PROJECT_TYPE
was extracted in Phase 0, map it to a focus area:

| Workspace signal | Focus area |
|-----------------|------------|
| USER_ROLE contains "product manager" or "product" | Product Management |
| USER_ROLE contains "project manager" or "delivery" | Project Management |
| USER_ROLE contains "program manager" or "program" | Program Management (show both product + project docs) |
| USER_ROLE contains "operations" or "ops" | Project Management |
| PROJECT_TYPE is [LAUNCH] | Product Management |
| PROJECT_TYPE is [CLIENT] | Project Management |
| PROJECT_TYPE is [PROGRAM] | Program Management |
| PROJECT_TYPE is [OPS] | Project Management |

If auto-detected, confirm with the user:
- **Question:** "Based on your workspace, it looks like you're focused on [detected focus area]. Want to see documents for that, or something different?"
- **Options:**
  - "[Detected focus area]" → "Show documents for [area]"
  - "Product management" → "Roadmaps, PRDs, strategy, competitive analysis, user stories"
  - "Project management" → "Charters, scope plans, WBS, risk plans, schedules, status reports"
  - "Show me everything" → "All document types across product and project management"

**If no workspace context:** Ask directly:
- **Question:** "What type of work are you planning?"
- **Options:**
  - "Product management" → "Building or managing a product — roadmaps, PRDs, strategy, user research"
  - "Project management" → "Delivering a project — charters, schedules, risk plans, status reports"
  - "Program management" → "Coordinating across projects — I need docs from both areas"
  - "Not sure yet" → "Show me everything and I'll pick"

Store the selection as FOCUS_AREA.

### Step 1b — Check for existing documents

If `docs/` has existing documents, include "Update an existing document"
as an option in the document picker below.

### Step 1c — Ask what to draft

Show only the documents relevant to the user's FOCUS_AREA. Use AskUserQuestion.

**If FOCUS_AREA is "Product Management":**

**Question:** "What would you like to draft?"
**Options:**
...
**If FOCUS_AREA is "Project Management":**
...
**If FOCUS_AREA is "Program Management" or "Show me everything":**
